"""Live-API integration tests for the Perplexity AI-backed naming pipeline.

Skipped by default — run with `pytest --real` and PERPLEXITY_API_KEY set.
Makes real calls to the Perplexity API (costs tokens).
"""
...
def test_build_pattern_matches_its_own_command(perplexity_key: str) -> None:
    builder = PerplexityRegexBuilder(api_key=perplexity_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(perplexity_key: str) -> None:
    builder = PerplexityRegexBuilder(api_key=perplexity_key)
...
def test_cli_name_end_to_end_fixed_command(perplexity_key: str, tmp_path: Path) -> None:
    builder = PerplexityRegexBuilder(api_key=perplexity_key)
...
def test_cli_name_end_to_end_is_cached_on_second_call(
    perplexity_key: str, tmp_path: Path
) -> None:
    index_path = tmp_path / ".cli-name.json"
    builder = PerplexityRegexBuilder(api_key=perplexity_key)
