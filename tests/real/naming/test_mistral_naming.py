"""Live-API integration tests for the Mistral AI-backed naming pipeline.

Skipped by default — run with `pytest --real` and MISTRAL_API_KEY set.
Makes real calls to the Mistral API (costs tokens).
"""
...
from parseforge.naming import CliContext, MistralRegexBuilder, cli_name
...
def test_build_pattern_matches_its_own_command(mistral_key: str) -> None:
    builder = MistralRegexBuilder(api_key=mistral_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(mistral_key: str) -> None:
    builder = MistralRegexBuilder(api_key=mistral_key)
...
def test_cli_name_end_to_end_fixed_command(mistral_key: str, tmp_path: Path) -> None:
    builder = MistralRegexBuilder(api_key=mistral_key)
