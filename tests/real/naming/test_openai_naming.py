"""Live-API integration tests for the OpenAI-backed naming pipeline.

Skipped by default — run with `pytest --real` and OPENAI_API_KEY set.
Makes real calls to the OpenAI API (costs tokens).
"""
...
from parseforge.naming import CliContext, OpenAIRegexBuilder, cli_name
...
def test_build_pattern_matches_its_own_command(openai_key: str) -> None:
    builder = OpenAIRegexBuilder(api_key=openai_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(openai_key: str) -> None:
    builder = OpenAIRegexBuilder(api_key=openai_key)
...
def test_cli_name_end_to_end_fixed_command(openai_key: str, tmp_path: Path) -> None:
    builder = OpenAIRegexBuilder(api_key=openai_key)
