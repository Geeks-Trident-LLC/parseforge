"""Live-API integration tests for the Together AI-backed naming pipeline.
...
from parseforge.naming import CliContext, TogetherRegexBuilder, cli_name
...
def test_build_pattern_matches_its_own_command(together_key: str) -> None:
    builder = TogetherRegexBuilder(api_key=together_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(together_key: str) -> None:
    builder = TogetherRegexBuilder(api_key=together_key)
...
def test_cli_name_end_to_end_fixed_command(together_key: str, tmp_path: Path) -> None:
    builder = TogetherRegexBuilder(api_key=together_key)
...
def test_cli_name_end_to_end_is_cached_on_second_call(
    together_key: str, tmp_path: Path
) -> None:
    index_path = tmp_path / ".cli-name.json"
    builder = TogetherRegexBuilder(api_key=together_key)
