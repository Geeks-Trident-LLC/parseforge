"""Live-API integration tests for the Groq-backed naming pipeline.

Skipped by default — run with `pytest --real` and GROQ_API_KEY set.
Makes real calls to the Groq API (costs tokens).
"""
...
from parseforge.naming import CliContext, GroqRegexBuilder, cli_name
...
def test_build_pattern_matches_its_own_command(groq_key: str) -> None:
    builder = GroqRegexBuilder(api_key=groq_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(groq_key: str) -> None:
    builder = GroqRegexBuilder(api_key=groq_key)
...
def test_cli_name_end_to_end_fixed_command(groq_key: str, tmp_path: Path) -> None:
    builder = GroqRegexBuilder(api_key=groq_key)
