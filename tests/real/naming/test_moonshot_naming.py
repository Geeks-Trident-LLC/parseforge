"""Live-API integration tests for the Moonshot AI (Kimi)-backed naming pipeline.

Skipped by default — run with `pytest --real` and MOONSHOT_API_KEY set.
Makes real calls to the Moonshot API (costs tokens).
"""
...
def test_build_pattern_matches_its_own_command(moonshot_key: str) -> None:
    builder = MoonshotRegexBuilder(api_key=moonshot_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(moonshot_key: str) -> None:
    builder = MoonshotRegexBuilder(api_key=moonshot_key)
...
def test_cli_name_end_to_end_fixed_command(moonshot_key: str, tmp_path: Path) -> None:
    builder = MoonshotRegexBuilder(api_key=moonshot_key)
...
def test_cli_name_end_to_end_is_cached_on_second_call(
    moonshot_key: str, tmp_path: Path
) -> None:
    index_path = tmp_path / ".cli-name.json"
    builder = MoonshotRegexBuilder(api_key=moonshot_key)
