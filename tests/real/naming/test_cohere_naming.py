"""Live-API integration tests for the Cohere-backed naming pipeline.

Skipped by default — run with `pytest --real` and COHERE_API_KEY set.
Makes real calls to the Cohere API (costs tokens).
"""
...
from parseforge.naming import CliContext, CohereRegexBuilder, cli_name
...
def test_build_pattern_matches_its_own_command(cohere_key: str) -> None:
    builder = CohereRegexBuilder(api_key=cohere_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(cohere_key: str) -> None:
    builder = CohereRegexBuilder(api_key=cohere_key)
...
def test_cli_name_end_to_end_fixed_command(cohere_key: str, tmp_path: Path) -> None:
    builder = CohereRegexBuilder(api_key=cohere_key)
