"""Live-API integration tests for the Oracle Cloud Infrastructure (OCI)
Generative AI-backed naming pipeline.
...
def test_build_pattern_matches_its_own_command(
    oci_compartment_id: str, oci_region: str
) -> None:
    builder = OCIRegexBuilder(compartment_id=oci_compartment_id, region=oci_region)
...
def test_cli_name_end_to_end_fixed_command(
    oci_compartment_id: str, oci_region: str, tmp_path: Path
) -> None:
    builder = OCIRegexBuilder(compartment_id=oci_compartment_id, region=oci_region)
