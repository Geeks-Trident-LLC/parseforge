"""Live-API integration tests for the Google Gemini-backed naming pipeline.

Skipped by default — run with `pytest --real` and GEMINI_API_KEY set.
Makes real calls to the Gemini API (costs tokens).
"""
...
def test_build_pattern_matches_its_own_command(gemini_key: str) -> None:
    builder = GeminiRegexBuilder(api_key=gemini_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(gemini_key: str) -> None:
    builder = GeminiRegexBuilder(api_key=gemini_key)
...
def test_cli_name_end_to_end_fixed_command(gemini_key: str, tmp_path: Path) -> None:
    builder = GeminiRegexBuilder(api_key=gemini_key)
