"""Live-API integration tests for the xAI (Grok)-backed naming pipeline.

Skipped by default — run with `pytest --real` and XAI_API_KEY set.
Makes real calls to the xAI API (costs tokens).
"""
...
from parseforge.naming import CliContext, XAIRegexBuilder, cli_name
...
def test_build_pattern_matches_its_own_command(xai_key: str) -> None:
    builder = XAIRegexBuilder(api_key=xai_key)
    response = builder.build_pattern("show version", CONTEXT)
    assert response.ready
    assert re.fullmatch(response.content, "show version", re.I)


def test_build_pattern_handles_a_variable_token(xai_key: str) -> None:
    builder = XAIRegexBuilder(api_key=xai_key)
...
def test_cli_name_end_to_end_fixed_command(xai_key: str, tmp_path: Path) -> None:
    builder = XAIRegexBuilder(api_key=xai_key)
