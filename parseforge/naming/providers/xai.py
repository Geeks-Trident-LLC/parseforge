"""xAI (Grok)-backed RegexBuilder implementation.

xAI's API mirrors OpenAI's chat.completions surface (same request/
response format), so this uses the `openai` SDK pointed at xAI's base
URL rather than a dedicated xAI client.
"""
...
DEFAULT_MODEL = default_model("xai")
...
def _import_openai() -> Any:
    """Deferred import — openai is an optional extra (parseforge[xai]);
    ...
    try:
        ...
        raise ImportError(
            "the openai package is required to use XAIRegexBuilder — "
            "install it via `pip install parseforge[xai]`"
        ) from exc
    return openai


class XAIRegexBuilder:
    """Builds a cli-name regex pattern by prompting a Grok model.
    ...
    requiring ``XAI_API_KEY`` to be set for cache-hit lookups, which
    never reach the LLM at all (see resolver.cli_name).

    API key resolves from the ``api_key`` argument if given, otherwise the
    ``XAI_API_KEY`` environment variable (unlike Anthropic/OpenAI's own
    SDKs, the ``openai`` package has no built-in notion of xAI's key, so
    ...
    def _get_client(self) -> openai_sdk.OpenAI:
        if self._client is None:
            ...
            if not api_key:
                raise RuntimeError("no xAI API key — pass api_key or set XAI_API_KEY")
