"""OpenRouter-backed RegexBuilder implementation.

OpenRouter is a model aggregator/router that re-exposes models from many
upstream providers under a single "vendor/model" namespace, but its API
mirrors OpenAI's chat.completions surface (same request/response format),
so this uses the `openai` SDK pointed at OpenRouter's base URL rather than
a dedicated OpenRouter client.
"""
...
def _import_openai() -> Any:
    """Deferred import — openai is an optional extra (parseforge[openrouter]);
    ...
    try:
        ...
        raise ImportError(
            "the openai package is required to use OpenRouterRegexBuilder — "
            "install it via `pip install parseforge[openrouter]`"
        ) from exc
    return openai


class OpenRouterRegexBuilder:
    """Builds a cli-name regex pattern by prompting an OpenRouter-routed model.
    ...
    requiring ``OPENROUTER_API_KEY`` to be set for cache-hit lookups, which
    never reach the LLM at all (see resolver.cli_name).

    API key resolves from the ``api_key`` argument if given, otherwise the
    ``OPENROUTER_API_KEY`` environment variable (unlike Anthropic/OpenAI's
    own SDKs, the ``openai`` package has no built-in notion of OpenRouter's
