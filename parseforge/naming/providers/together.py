"""Together AI-backed RegexBuilder implementation.

Together's API mirrors OpenAI's chat.completions surface (same request/
response format), so this uses the `openai` SDK pointed at Together's
base URL rather than a dedicated Together client.
"""
...
DEFAULT_MODEL = default_model("together")
...
def _import_openai() -> Any:
    """Deferred import — openai is an optional extra (parseforge[together]);
    ...
    try:
        ...
        raise ImportError(
            "the openai package is required to use TogetherRegexBuilder — "
            "install it via `pip install parseforge[together]`"
        ) from exc
    return openai


class TogetherRegexBuilder:
    """Builds a cli-name regex pattern by prompting a Together-hosted model.
    ...
    requiring ``TOGETHER_API_KEY`` to be set for cache-hit lookups, which
    never reach the LLM at all (see resolver.cli_name).

    API key resolves from the ``api_key`` argument if given, otherwise the
    ``TOGETHER_API_KEY`` environment variable (unlike Anthropic/OpenAI's
    own SDKs, the ``openai`` package has no built-in notion of Together's
    ...
    def _get_client(self) -> openai_sdk.OpenAI:
        if self._client is None:
            ...
            if not api_key:
                raise RuntimeError(
                    "no Together API key — pass api_key or set TOGETHER_API_KEY"
                )
