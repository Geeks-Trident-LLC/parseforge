"""Moonshot AI (Kimi)-backed RegexBuilder implementation.

Moonshot's API mirrors OpenAI's chat.completions surface (same request/
response format), so this uses the `openai` SDK pointed at Moonshot's
base URL rather than a dedicated Moonshot client. Uses the global
api.moonshot.ai endpoint (not the China-only api.moonshot.cn endpoint,
which bills in RMB).
"""
...
DEFAULT_MODEL = default_model("moonshot")
...
def _import_openai() -> Any:
    """Deferred import — openai is an optional extra (parseforge[moonshot]);
    ...
    try:
        ...
        raise ImportError(
            "the openai package is required to use MoonshotRegexBuilder — "
            "install it via `pip install parseforge[moonshot]`"
        ) from exc
    return openai


class MoonshotRegexBuilder:
    """Builds a cli-name regex pattern by prompting a Moonshot-hosted model.
    ...
    requiring ``MOONSHOT_API_KEY`` to be set for cache-hit lookups, which
    never reach the LLM at all (see resolver.cli_name).

    API key resolves from the ``api_key`` argument if given, otherwise the
    ``MOONSHOT_API_KEY`` environment variable (unlike Anthropic/OpenAI's
    own SDKs, the ``openai`` package has no built-in notion of Moonshot's
    ...
    def _get_client(self) -> openai_sdk.OpenAI:
        if self._client is None:
            ...
            if not api_key:
                raise RuntimeError(
                    "no Moonshot API key — pass api_key or set MOONSHOT_API_KEY"
                )
