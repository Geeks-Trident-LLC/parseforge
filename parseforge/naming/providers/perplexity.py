"""Perplexity AI-backed RegexBuilder implementation.

Perplexity's API mirrors OpenAI's chat.completions surface (same request/
response format), so this uses the `openai` SDK pointed at Perplexity's
base URL rather than a dedicated Perplexity client.
"""
...
DEFAULT_MODEL = default_model("perplexity")
...
def _import_openai() -> Any:
    """Deferred import — openai is an optional extra (parseforge[perplexity]);
    ...
    try:
        ...
        raise ImportError(
            "the openai package is required to use PerplexityRegexBuilder — "
            "install it via `pip install parseforge[perplexity]`"
        ) from exc
    return openai


class PerplexityRegexBuilder:
    """Builds a cli-name regex pattern by prompting a Perplexity-hosted model.
    ...
    requiring ``PERPLEXITY_API_KEY`` to be set for cache-hit lookups, which
    never reach the LLM at all (see resolver.cli_name).

    API key resolves from the ``api_key`` argument if given, otherwise the
    ``PERPLEXITY_API_KEY`` environment variable (unlike Anthropic/OpenAI's
    own SDKs, the ``openai`` package has no built-in notion of Perplexity's
    ...
    def _get_client(self) -> openai_sdk.OpenAI:
        if self._client is None:
            ...
            api_key = self._api_key or os.environ.get("PERPLEXITY_API_KEY")
            if not api_key:
                raise RuntimeError(
                    "no Perplexity API key — pass api_key or set PERPLEXITY_API_KEY"
                )
