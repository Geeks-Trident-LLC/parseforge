...
`groq`, or `xai`. `--provider` defaults to `anthropic` wherever it isn't
...
as needed, e.g.
`pip install parseforge[anthropic,openai,deepseek,groq,xai,sampling]`.
...
all five providers — `anthropic`, `openai`, `deepseek`, `groq`, `xai` share
just those two packages — and never silently skip) — add
